...
class WR_MissionSystemConfig
{
	...
	[Attribute(defvalue: "1", uiwidget: UIWidgets.Slider, params: "0.01 60 0.01", desc: "(minutes) How long to wait to add a new mission to the queue after one ends.")]
	float m_fNewMissionDelayM;
	
	[Attribute(defvalue: "3", uiwidget: UIWidgets.Slider, params: "0 60 0.01", desc: "(minutes) How long to wait to start the mission after displaying the 'Upcoming Objective' notification. \n\nExample: 0 -> The 'Upcoming Objective' notifications will not be displayed.\nExample: 5 -> The 'Upcoming Objective' notifications will be displayed upon mission creation and the mission will start 5 minutes later.")]
	float m_fNewMissionNotificationDelayM;
	
	[Attribute(defvalue: "1", uiwidget: UIWidgets.Slider, params: "0 5 0.1", desc: "(minutes) How long to wait to delete a mission's map marker after it ends.")]
	...
	[Attribute(defvalue: "40", uiwidget: UIWidgets.Slider, params: "0 120 1", desc: "(minutes) Time limit for missions. \n\n Example: 0 -> no time limit")]
	float m_fMissionTimeLimitM;
	
	[Attribute(defvalue: "10", uiwidget: UIWidgets.Slider, params: "0 128 1", desc: "Server must have at least this many players to spawn missions of 'free' difficulty")]
	int m_iFreeMissionThreshold;
